...
"""
REFUTED context-compression modes — kept only so existing imports and the
historical benchmark suite keep working. **Nothing in this module is on the
serving path.** The live scorer is `vllm.entrypoints.context_compression`.

Why this code was retired
-------------------------
Measured on LoCoMo (200 questions, the benchmark's own F1, a real model
answering, same ~8% budget). Higher is better:

    full context (ceiling) ........ 22.56
    tail truncation (floor) ........ 6.62
    THIS FILE's heuristic scorer .... 6.07   <-- BELOW the do-nothing floor
    BM25 without IDF .............. 20.34
    BM25 .......................... 23.59
    embeddings .................... 23.95
    weighted-sum hybrid ........... 26.36
    reciprocal RANK fusion ........ 28.09

Two results decided the rewrite:

  * The hand-written content rules in `_heuristic_score` are not merely
    unhelpful, they are NEGATIVE: turning them on costs **-3.02 F1**
    (26.36 with weight 0 -> 23.34 with the weight this file shipped). Rules
    about which words matter ("error" = 0.95, a path = 0.9) outweighed what
    the user is actually asking about.

  * Dropping IDF costs -3.25 F1 (23.59 -> 20.34). `_BM25Scorer` below
    computes IDF over the lines of a *single message*, so the document
    frequencies come from a corpus of a few dozen near-identical lines and
    carry almost no signal.

The attention mode (Mode 3) never beat BM25 in any measured configuration and
requires a per-request attention-capture hook, a client-supplied
`conversation_id` keyed into a process-global registry, and single-tenancy. It
is retained here unchanged, unused.

Everything below is the pre-rewrite implementation, behaviourally unchanged
(only reformatted), so the recorded numbers above stay reproducible from this
tree.
"""
...
class AttentionImportanceTracker:
    ...
    def score_lines(self, token_spans: list[tuple[int, int]]) -> list[float]:
        """
...
# Populated by the render path when context_compression="ace" and
...
def _heuristic_score(line: str) -> float:
    ...
    if any(
        p in sl
        for p in ["error", "failed", "not found", "traceback", "exception", "exit code"]
    ):
        return 0.95
    ...
    if any(
        k in sl
        for k in ["here are the results", "i executed", "tool results:", "output:"]
    ):
        return 0.1
    return 0.5
...
class _BM25Scorer:
    ...
    def score_lines(self, query: str, lines: list[str]) -> list[float]:
        ...
        tokenized = [self._tok(line) for line in lines]
        n = len(lines)
        df: dict[str, int] = {}
...
def apply_ace_eviction(
    ...
    query = (
        ...
        if (use_query_relevance and not use_attention)
        else None
    )
    ...
    n_comp = max(len(compressible) - 1, 1)
    saved = 0

    for rank, idx in enumerate(compressible):
        if _total(messages) <= budget_chars:
            break
        ...
        if recency_blend > 0:
            ...
            if attn_scores is not None and len(attn_scores) == len(lines):
                ...
                raw = [_heuristic_score(line) for line in lines]
